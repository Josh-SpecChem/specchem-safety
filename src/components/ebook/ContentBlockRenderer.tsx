...
export function ContentBlockRenderer({ 
  ...
  switch (block.blockType) {
        ...
        >
          {content.icon && (
            <div className="flex justify-center mb-4">
              {renderIcon(content.icon as string, `h-12 w-12 sm:h-16 sm:w-16 ${content.iconColor || 'text-gray-600'}`)}
            </div>
          )}
        ...
        >
          {content.title && (
            <CardHeader className={content.headerClass}>
              <CardTitle className={`flex items-center gap-2 ${content.titleClass || ''}`}>
                {content.icon && renderIcon(content.icon, `h-6 w-6 ${content.iconColor || ''}`)}
                {content.title}
              </CardTitle>
            </CardHeader>
          )}
    ...
    case 'callout':
      ...
      const variantClass = calloutVariants[variant as keyof typeof calloutVariants] || calloutVariants.info;
                ...
                <div className="space-y-2 mt-3">
                  {item.features.map((feature: any, fIndex: number) => (
                    <div key={fIndex} className="text-sm">
                      <span className="font-medium text-gray-900">{feature.title || feature}</span>
                      {feature.spec && (
